...
def compute_metrics(df: pd.DataFrame, score_col: str = 'posegate_score',
                     alpha: float = 20.0, ef_fraction: float = 0.01):
    """Point-estimate AUC-ROC, EF at ef_fraction, and BEDROC(alpha) for one
    (possibly resampled) active/decoy set. Lower score_col = more
    active-like (matches both posegate_score and raw vina_score's own
    convention), so it's ranked ascending / negated for AUC."""
    ranked = df.sort_values(score_col, ascending=True)
    scores_col = [[1.0, int(label)] for label in ranked['label']]
    auc = roc_auc_score(ranked['label'], -ranked[score_col])
    ef = CalcEnrichment(scores_col, 1, [ef_fraction])[0]
    bedroc = CalcBEDROC(scores_col, 1, alpha=alpha)
    return auc, ef, bedroc

def stratified_bootstrap(df: pd.DataFrame, n_boot: int, score_col: str, alpha: float,
                          ef_fraction: float, seed: int):
    ...
    for _ in range(n_boot):
        ...
        try:
            auc, ef, bedroc = compute_metrics(sample, score_col=score_col, alpha=alpha, ef_fraction=ef_fraction)
        except Exception:
            continue
        aucs.append(auc)
        efs.append(ef)
...
def main():
    ...
    parser.add_argument("--score_col", default="posegate_score",
                        help="Column to rank by, e.g. vina_score for the raw-docking baseline "
                             "instead of the fitted posegate_score")
    ...
    auc_point, ef_point, bedroc_point = compute_metrics(
        df, score_col=args.score_col, alpha=args.alpha, ef_fraction=args.ef_fraction)
    print(f"Scoring by: {args.score_col}")
    ...
    aucs, efs, bedrocs = stratified_bootstrap(
        df, args.n_boot, args.score_col, args.alpha, args.ef_fraction, args.seed)
